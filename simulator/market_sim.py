...
def backtest(experiment_id, trading_hyperparameters):
    prices, sanity_check_labels, targets, predictions, probs = __get_data__(
        experiment_id
    )
    ...
    mask = (prices["Predictions"] != prices["Predictions"].shift()) | (
        prices.index == 0
    )
    prices = prices[mask]
    ...
    for i in tqdm(range(len(predictions))):
        ...
        if trading_hyperparameters["mid_side_trading"] == "mid_to_mid":
            if i in end_of_day_indices:
                ...
                if (
                    prediction == 2
                    and probability >= trading_hyperparameters["probability_threshold"]
                ):
                    if (
                        TradingAgent.long_inventory == 0
                        and TradingAgent.short_inventory == 0
                    ):
                    ...
                    if (
                        ...
                        TradingAgent.short_inventory == 0
                        and TradingAgent.long_inventory > 0
                    ):
                        TradingAgent.exit_long(mid_price, timestamp)
                        TradingAgent.short(mid_price, timestamp)
